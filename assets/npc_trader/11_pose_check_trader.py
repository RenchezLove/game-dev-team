"""Deformation check for the trader: load _work/trader_work.blend, render the
assembled set at rest + in a bent pose (elbows/shoulder/knee/neck) — the x100
lesson: if the rig bones were baked wrong the added bandana/pouches/soles would
explode off the body when posed. Also a wireframe pass. Output: renders_trader/.
Run: blender.exe -b --factory-startup --python 11_pose_check_trader.py
"""
...
OUT = HERE + 'renders_trader/'
os.makedirs(OUT, exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=HERE + '_work/trader_work.blend')
sc, cam, suns = W.setup_render(res=860)
rig = bpy.data.objects['RootAnim']
SET = [bpy.data.objects[n] for n in ('SK_Trader_Head', 'SK_Trader_Torso', 'SK_Trader_Legs')]
...
# ---- lit assembled (rest pose) ----
show_only(SET)
for vn, vd in VIEWS.items():
    W.frame_and_shoot(SET, vd, OUT + 'Trader_assembled_%s.png' % vn, suns=suns)

# ---- posed: bend joints (same pose as the elder / wearables batches) ----
POSE = {
...
for vn in ('front', 'tq', 'side'):
    W.frame_and_shoot(SET, VIEWS[vn], OUT + 'Trader_pose_%s.png' % vn, suns=suns)
