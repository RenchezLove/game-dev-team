...
CMD = (
    ...
    "    if lbl in ('POI1_Loot', 'POI2_Loot', 'BP_Pickup'):\n"
    "        cls_name = a.get_class().get_name()\n"
    "        item = a.get_editor_property('PlacedItemClass')\n"
    "        ammo = a.get_editor_property('PlacedAmmoAmount')\n"
    "        money = a.get_editor_property('MoneyAmount')\n"
    "        print('PROPS|%s|class=%s|PlacedItemClass=%s|PlacedAmmoAmount=%s|MoneyAmount=%s' % ("
    "lbl, cls_name, item.get_name() if item else 'None', ammo, money))\n"
)
