...
## ADR-004 — concept-artist: визуальный инструмент (ПЕРЕСМОТРЕН — нативный code-render вместо canvas-design)
...
- **(d)** **Ограничение (нативно не закрывается):** фигуративные/живописные концепт-зарисовки (детальный рисунок персонажа/сцены «от руки») требуют генератора изображений, которого в нативном тулсете нет. Code-render силён в палитрах, мудбордах из плашек/типографики, силуэтах, блокаутах, UI — но не в фигуративном рисунке. Такие задачи `concept-artist` эскалирует `game-lead`, не имитирует.

**Статус: ПЕРЕСМОТРЕН — нативный code-render (Edge headless), валидирован живым рендером Ш7 (2026-06-08).** См. также инструкцию роли `.claude/agents/concept-artist.md`.
