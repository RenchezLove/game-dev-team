...
## 🧭 СЕЙЧАС
**КАМЕРА-ПАРАМЕТРЫ НАВЕРХ Details — СБОРКА PASS** (`logs/phaseB-camera-displaypriority-build.log`, BUILD_EXIT=0, релинк DLL при закрытом редакторе, ветка feature/clean-village). По фидбеку Рината: всем EditAnywhere-параметрам камеры в `PlayerCharacter.h` добавил `meta=(DisplayPriority=...)` (только .h, ClampMin сохранён, логика/.cpp не тронуты, НЕ коммитил). Camera: ArmLength=10, FieldOfView=11, BoomRotation=12, bEnableCameraLag=13, CameraLagSpeed=14, CameraLagMaxDistance=15. Camera|Feel: bEnableCameraBreathing=16, BreathingAmplitude=17, BreathingSpeed=18, bEnableCameraLookAhead=19, LookAheadAmount=20, LookAheadInterpSpeed=21, LookAheadSpeedThreshold=22. SpringArm(DP=1)/CameraComponent не трогал.
- **МЕХАНИЗМ DisplayPriority (проверено по движку UE5.5, PropertyEditorHelpers.cpp:1284 OrderPropertiesFromMetadata):** сортирует ВСЕ свойства класса по DisplayPriority по возрастанию (меньше=выше, без метки=MAX_int32 в конец), потом группирует по категориям. Порядок категорий сверху = МИНИМАЛЬНЫЙ DisplayPriority в категории. Camera уже первая (SpringArm DP=1). Взял 10-22 (выше 1-5), чтобы не задеть порядок Stats=2/Quest=3/Nav=4/Save=5. «Camera|Feel» с «|» = подкатегория ПОД Camera.
- **ВИЗУАЛ порядка в Details = UI, сборкой НЕ доказывается** — финально подтверждает game-lead/Ринат живым взглядом в редакторе.
- **УРОК (фича дальности отрисовки мертва — выпилена Ринатом, опираемся на встроенный frustum culling движка):** UDrawFrustumComponent (если вернёмся) рисуется ТОЛЬКО при show-флаге вьюпорта Camera Frustums (DrawFrustumComponent.cpp:124; флаг дефолтно OFF, ShowFlags.h:424). Имя локальной переменной НЕ `Mesh` в наследниках ACharacter (шадовит `ACharacter::Mesh` → C4458, модуль с -WarningsAsErrors).
