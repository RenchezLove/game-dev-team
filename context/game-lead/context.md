...
## 🧭 СЕЙЧАС (2026-08-06 — задание издателя Б1–Б10, вторая волна)

**ФАЗА:** доводка перед первым выходом на RuStore. Ветка `feature/build122`, в master не вливалось. Порядок работ — ADR-059.

**ЗАКРЫТО РАНЬШЕ (проверено лично):** Б1 (ассеты в пакете + анимация смерти живьём), Б2 и Б7 (продажа стопками, экономика, старт с ножом — видел в рюкзаке), Б3 (загрузка сейва при старте — подтверждено кадром `Screenshots/phone-b7-02-continue.png`: старт у костра с монетами), Б9 (соотношение сторон + рисование в область выреза). Вопрос кадров ЗАКРЫТ решением Рината 08-06: 24 кадра на слабом телефоне его устраивают, оптимизацию видеочипа не ведём.

**ЗАКРЫТО 08-06, ПРОВЕРЕНО МНОЙ ЛИЧНО ПО ЛОГАМ:**
- **Б4 аналитика** (`b98ed0d`): ключи вшиваются при компиляции через `ContrarySurvivor.Build.cs` из `E:/game-dev-team/keys` (папка в .gitignore), переопределяется переменной `CONTRARY_ANALYTICS_KEYS_DIR`; новые события `app:first_launch`, `tutorial:step:<шаг>`, `tutorial:completed`. Тесты 55/0 (`Saved/b4-analytics-tests1.log`), ключей нет ни в дереве, ни в истории (искал прямым сравнением). Живая доставка событий на сервер — НЕ ПРОВЕРЕНА.
- **Подсказка взаимодействия** (`86cc51f`, задача Рината): теперь «Обыскать — ДЕЙСТВИЕ» на телефоне и «Обыскать — E» на компьютере; действия отдельными редактируемыми полями, у старосты дословно «Поговорить со старостой». Клавиша сверена с `Config/DefaultInput.ini:14`, подпись кнопки — с `ContrarySurvivorPlayerController.cpp:69`.
- **Подписи клавиш ПК на телефоне спрятаны** (`dcd4349`): заголовок инвентаря, подсказка магазина, строка возрождения на экране смерти.
- **Экран смерти переписан** (`eb79650`): одна мысль про деньги под каждой кнопкой, «Тебя убил волк» / «Ты умер от жажды» / «Ты не пережил эту вылазку», строка предметов прячется при нуле, склонение по числу. Тексты УТВЕРЖДЕНЫ мной.
- Тесты по этой волне 57/0 (`Saved/b122-prompt-tests1.log`), сборка `Saved/b122-prompt-build4.log` — 0 ошибок.
- **Тексты Б6 написаны мной** и закоммичены в team-репо (`5d484c9`, `323b3d2`): `docs/contrary-survivor/soglasie-i-politika.md` — источник истины по экрану согласия, и готовая страница `docs/contrary-survivor/privacy/index.html`. Состав сборщиков данных сверен с реальными файлами сборки: Яндекс 8.2.0, посредник VK 5.45.3.3, myTarget 5.45.3, AppMetrica 8.3.0 (`YandexAds_UPL.xml:45-58`), GameAnalytics 7.0.1 (`GameAnalytics_APL.xml:34`); разрешения — только интернет и состояние сети.

**▶ В РАБОТЕ (трое):** `cpp` — кодовая часть Б5 (вырезать отладочные клавиши из Shipping, проверить счётчик кадров, адрес кнопки «Написать мне» в конфиг); `cpp-4` — Б8 вёрстка магазина + полосы статов из-под выреза + подпись «Закрыть (Tab)» в генераторе; `cpp-3` — Б6 экран согласия и политика по моему документу.

**▶ ЖДЁТ РИНАТА:** (1) создать ОТДЕЛЬНОЕ открытое хранилище `contrarysurvivor-privacy` на GitHub и включить в нём Pages — репозиторий игры остаётся закрытым, публикуем только страницу политики (он сказал «да, кидай на github», сеть из моей оболочки закрыта, `curl` заблокирован); (2) данные ИП и почта для политики — пока оставлены пустыми по его решению; (3) три боевых блока рекламы из кабинета (Б10); (4) русское название игры для RuStore.

**▶ ОСТАЛОСЬ ПОСЛЕ ТЕКУЩЕЙ ВОЛНЫ:** сама упаковка Shipping оператором (Б5), живая проверка на телефоне, qa-гейт, мерж в master.

**⚠️ УРОКИ 08-06:** (а) файл правил сборки `ContrarySurvivor.Build.cs` — общий ресурс: сломанный, он валит сборку ВСЕМ троим раньше любого C++ (было: `Logger.LogInformation` без `using Microsoft.Extensions.Logging;`); правку в нём проверять сборкой сразу. (б) Гонка в общем дереве: `.cpp` соседа записан раньше своего `.h` → чужие ошибки в моей сборке, лечится ожиданием ~45 секунд. (в) Замок `.build-lock` работает, исполнители его соблюдают. (г) Обрыв связи у исполнителя — наработки спасать WIP-коммитом по его точным путям (сделал: `42ffc2c`, 305 строк).
