...
## ⚡ РЕЖИМ АВТОНОМИИ (директива Рината 2026-06-15, ДЕЙСТВУЕТ ПОСТОЯННО)
- **«Меня не спрашивайте. Работайте самостоятельно. Если что — используй разумные дефолты. Считайте, что меня нет.»** — game-lead и Сборщик максимально автономны.
- НЕ упираться в ожидание ответа Рината по рутинным/тех-вопросам — принимать разумный дефолт, помечать допущение, двигаться дальше. Эскалировать ТОЛЬКО критичные дизайнерские развилки, и то — продолжая работу по дефолту.
- Канал со Сборщиком (КАНОН, единый): `reports/Agents/STATUS.md` (пишу я) + `TEST_REPORT.md` (Сборщик) + `QA_TESTING.lock` (на время теста). Файлы НА ДИСКЕ в рабочем дереве. НЕ GL_SIGNAL/QA_SIGNAL (устарело).
- CU для live-PIE: Сборщик берёт сам (грант проходит молча, проверено 00:15). Если CU не доходит — продолжать CU-free работу (сборка/headless/визуал), тест накапливать.
- **🔴 У МЕНЯ ПОЛНЫЕ ПОЛНОМОЧИЯ И «РУКИ» (директива Рината 2026-06-15, КАПСОМ):** game-lead САМ запускает UnrealEditor.exe (`/e/UnrealEngine/UE_5.5/Engine/Binaries/Win64/UnrealEditor.exe "<uproject>"` фоном), САМ гоняет headless-коммандлеты (`UnrealEditor-Cmd ... -ExecutePythonScript`), САМ собирает (Build.bat), САМ правит уровень/ассеты/код через специалистов или напрямую. НЕ спрашивать у Рината разрешения на запуск редактора/сборку/правки — это рутина, делать молча. Спрашивать Рината — практически НИКОГДА (только реальная дизайн-развилка, и то продолжая по дефолту). ПЕРЕСТАТЬ писать «нужен ты / нужно CU-окно / давать добро?» — просто делать.

## ⚙️ ПРОЦЕСС-ПРАВИЛА ПАРАЛЛЕЛИЗМА (директива Рината 2026-06-15, КАПСОМ — записать накрепко)
- **ВСЕГДА держать в работе до 3 агентов параллельно.** НЕ гонять по одному — команда не должна простаивать. Дал задание одному → сразу загрузи остальных независимой работой.
- **Отдавать готовую волну Сборщику на тест СРАЗУ, не копить и не сериализовать за разработкой.** Пока Сборщик тестит — команда делает СЛЕДУЮЩУЮ волну. (Мой косяк: поставил Волну 1 на hold вместо «QA тестит ↔ команда работает».)
- **Единственный конфликт — один редактор/проект:** нельзя одновременно live-PIE (Сборщик) и headless-правку ТОГО ЖЕ `.umap` (unreal). Поэтому overlap делать так: **Сборщик тестит ↔ cpp кодит ↔ modeler в Blender** (эти три не конфликтуют). unreal-правка УРОВНЯ сериализуется с QA-тестом через `QA_TESTING.lock`. Импорт/правка АССЕТОВ (не .umap) — можно фоном между тестами.
- Практика: как только есть стабильный билд → 🟢 Сборщику + параллельно cpp/modeler на следующее. Не ждать конца теста, чтобы начать разработку.

## 🖼 ПРАВИЛО ВИЗУАЛ-ПРУФА (директива Рината 2026-06-15 — записать накрепко)
- **Любую визуалку в UE подтверждать РЕАЛЬНЫМ скрином ИЗ ДВИЖКА** (открыть редактор/HighResShot), НЕ headless-параметрами. Headless-коммандлет НЕ рендерит → «выставил emissive/exposure/decal size» ≠ «выглядит правильно».
- НЕ говорить «готово/исправлено» по визуалу без картинки из UE, которую я лично посмотрел.
- Blender-рендеры подтверждают только саму МОДЕЛЬ. Материалы/свет/экспозиция/декали/огонь — это UE-сторона, нужен UE-скрин.
- Прошлый прокол (BugReport 13): принял огонь/кровь-декали/экспозицию по headless-параметрам unreal без UE-скрина → костёр бирюзовит/пересвечен, декали — розовые прямоугольники. Не повторять.

## ⏰ БУДИЛЬНИКИ / КАНАЛЫ СВЯЗИ (директива Рината 2026-06-15 — перевзводить при старте сессии)
- **ДВА РАЗНЫХ КАНАЛА:** (1) **Ринат = git** (`origin/docs/phase0-gdd`): я пушу `reports/Agents/HEARTBEAT.md` + STATUS/TEST_REPORT, он смотрит с телефона. (2) **Сборщик = локальные файлы на ноуте**: я пишу STATUS.md, он пишет TEST_REPORT.md, лок QA_TESTING.lock.
- **БУДИЛЬНИК 1 — git-пульс Ринату КАЖДЫЕ 10 МИН, безусловно** (даже если ничего не происходит). Скрипт `reports/Agents/_gl_heartbeat_loop.sh` (фон, run_in_background). Публикует снапшот из `reports/Agents/gl_heartbeat_status.txt` (я его обновляю по событиям) + `git add reports/Agents/ && commit && push origin docs/phase0-gdd`.
- **БУДИЛЬНИК 2 — watcher TEST_REPORT.md каждые 60с**: `reports/Agents/_gl_testreport_watch.sh` (фон). Выходит (= будит меня нотификацией) при изменении TEST_REPORT.md → я читаю отчёт Сборщика, реагирую, ПЕРЕВЗВОЖУ watcher заново (одноразовый на срабатывание).
- **Сборщику пишу ПО СОБЫТИЮ** (готова волна → 🟢; реакция на его отчёт), НЕ по таймеру.
- **ВАЖНО при старте новой сессии:** оба фоновых будильника НЕ переносятся — поднять заново (`bash ..._gl_heartbeat_loop.sh` и `..._gl_testreport_watch.sh` в фоне). Обновить gl_heartbeat_status.txt текущим состоянием.
- НИКОГДА `git reset --hard`/`checkout --` по `reports/Agents/` (затрёт отчёты Сборщика). Только add/commit/push.
...
---
# === SAVE 2026-06-14 ~23:35 (перед лимитом) — НОЧНОЙ РЕЖИМ, точка возврата ===

## ⚠️ КАНАЛ СВЯЗИ — ИСПРАВЛЕНО (важно для новой сессии)
- Сборщик **НЕ может git push** (нет write-кредов: "could not read Username"). Он **читает/пишет ЛОКАЛЬНЫЕ файлы в дереве `E:\game-dev-team` напрямую, БЕЗ CU**.
- Канал game-lead↔Сборщик = **локальные файлы через `cat`**, НЕ git: я пишу `reports/Agents/GL_SIGNAL.txt`, он `reports/Agents/QA_SIGNAL.txt`. Читаем `cat`'ом. CU у Сборщика — ТОЛЬКО на тест (редактор/PIE).
- **git push — только я** (для телефона Рината); в пуш включать `reports/Agents/` целиком (отчёты Сборщика тоже). **НИКОГДА** `git reset --hard`/`checkout --` по `reports/Agents` (затрёт несохранённые отчёты Сборщика); перед pull коммитить report-файлы.
- Мутуальный пульс каждые 5-10 мин, формат `[ЧЧ:ММ] <роль>: <статус> | <деталь>`. Занятость редактора = строка Сборщика `testing <HEAD> (editor busy)` / `🔵 free` (lock-файл убран).
- Пульс Ринату на Git каждые ~8-10 мин (HEARTBEAT.md). Computer Use нужен ТОЛЬКО для теста Сборщиком (открыть редактор/PIE); его авто-таймер без апрува Рината глохнет.

## СОСТОЯНИЕ (гейм-репо ветка feature/phase5-quests)
- HEAD origin = `3094871` (навмеш Dynamic + invoker-off). Запушено (бэкап).
- В работе (cpp, мог докоммитить ПОСЛЕ 3094871 — проверь `git log`): **погоня-fallback** (`a9a6231a`): если MoveTo Failed/цель вне навмеша → прямой steering `AddMovementInput` к игроку (надёжно без навмеша). Диагностика MoveTo (selfNav/targetNav/result) уже в `159729d`.
- modeler: `SM_CampfireFlame.fbx` готов в `E:/ForGameLead(Materials)/demo-assets/` (НЕ импортирован).

## ✅ ВЕРИФИЦИРОВАНО Сборщиком (ранее)
- Кор: движение/бой/лут/авто-лок/спавн-на-полу. **2 квеста сквозняком** (шкуры+150, база+ноутбук+250). **Экран смерти #26**, **спринт-расход #2**.
## 🔴 ОТКРЫТО (главное)
- **Погоня не доходит** (a548272): враги входят в стейт, но не добегают — корень: игрок на телепортах V/Z садится floor-trace (ВНЕ навмеша) → путь не строится. ФИКС = погоня-fallback (a9a6231a, в работе). Проверить Сборщиком.
- Навмеш у базы/Логова динамический ненадёжен; статик-бейк = разовый **Build Paths в редакторе** (нужен CU/Ринат) — опция, если fallback мало.

## NEXT (ночь/утро, без CU)
1. Дождаться cpp погоня-fallback → импортировать `SM_CampfireFlame` + расставить на 2 костра + базу (headless) → дать огонь #19.
2. cpp: патроны-предмет+слайдер #3 → броня 2×3 #21 → HUD-читаемость/иконка #18.
3. qa: чистая сборка батча.
4. Лица #22 — утром с Ринатом (арт-ориентир).
5. Утром: большой 🟢-батч Сборщику (погоня + огонь + #3/#21/#18) на верификацию (нужен CU-окно от Рината) + сводка.
- DT_Quests/DT_Items (конструкторы) — ближе к концу демки (JIT), когда квесты устаканятся.
## DEBUG-КЛАВИШИ: F1 cam,F2 items,F3/F4 броня,F6 use,F7 drop,F9/F10 buy/sell,F12 clearsave,M +100,T торговец,Y староста,G взять квест,H сдать,K +волк,J god,U force-drop,B тест-волк вплотную,O оверлей,N force-kill ближнего,V телепорт-Логово,Z телепорт-база,C 5 шкур,X ноутбук,P убить игрока.

---
# === SAVE 2026-06-15 ~00:25 (КОНТЕКСТ 98% — НОВАЯ СЕССИЯ СТАРТУЕТ ОТСЮДА) ===

## ГЕЙМ-РЕПО: ветка feature/phase5-quests, HEAD origin = b1c3fcd (всё забэкаплено). master=627d243 НЕ тронут.
Коммиты этой ночи (поверх прошлого save): 5426944 (фикс движения BaseWalkSpeed=600) · a7a2a4e (огонь костра SM_CampfireFlame+M_Flame на все костры #19) · 159729d (диаг-лог погони MoveTo) · 224fbc9 (погоня-fallback direct steering) · 3f71b37 (#3 патроны-предмет AAmmoItem + слайдер магазина + перезарядка из рюкзака) · b1c3fcd (RecastNavMesh=STATIC под запекание).

## 🔴 СЛЕДУЮЩИЙ КРИТИЧЕСКИЙ ШАГ: ЗАПЕЧЬ НАВМЕШ
- RecastNavMesh переключён на **STATIC** (b1c3fcd), НО **навмеш ещё НЕ запечён** → сейчас пусто, надо **Build Paths**.
- ДЕЙСТВИЕ: Сборщик (у него ночью CU РАБОТАЕТ — грант прошёл молча ~00:07) ИЛИ Ринат: открыть редактор → меню **Build → Build Paths** → Save Level → закрыть. Это даст навмеш ВЕЗДЕ (деревня+Логово+база).
- ПОЧЕМУ: погоня FAIL (Сборщик, TEST_REPORT 00:15) — диаг-лог показал ПРЯМО: `selfNav=no targetNav=no moveResult=Failed` — и враг, И игрок ВНЕ навмеша на боевых точках (динамический навмеш у Логова/базы не строился надёжно, гонял 3 способа). Статик-бейк = окончательный фикс: игрок-телепорт(floor-trace) и враг-спавн сядут на запечённый навмеш → MoveTo работает → погоня. После бейка: перетест V→Логово/Z→база, ждать `chase mode=nav selfNav=yes targetNav=yes` + dist убывает + HP игрока падает.

## СВЯЗЬ (подтверждено ночью)
- Канал game-lead↔Сборщик = **ЛОКАЛЬНЫЕ файлы** reports/Agents/{GL_SIGNAL.txt(я), QA_SIGNAL.txt(он), TEST_REPORT.md(он)} через `cat`. Сборщик **НЕ может git push** (нет кредов) и **НЕ видит Source/** (только Saved/Logs + reports/Agents). git push — ТОЛЬКО я.
- Сборщик: scheduled task "Night Watch", CU выдан Ринатом — ночью CU СРАБАТЫВАЕТ (тест 00:15 прошёл). Мутуальный пульс 5-10 мин. Занятость редактора = строка в QA_SIGNAL (testing/free), НЕ lock-файл.
- НЕ делать git reset/checkout по reports/Agents (затрёт отчёты Сборщика); add reports/Agents/ целиком в пуш.

## ✅ ВЕРИФИЦИРОВАНО Сборщиком: экран смерти #26, спринт #2, 2 квеста (+150/+250), лут-цепочка, кор.
## ⏳ НЕ ВЕРИФИЦИРОВАНО (батч на проверку): движение-фикс 5426944, огонь a7a2a4e, #3 патроны-слайдер 3f71b37, ПОГОНЯ после бейка навмеша.

## NEXT по приоритету
1. **Build Paths (запечь навмеш)** → перетест погони. ← критический, разблокирует бой у зон.
2. Верифицировать движение/огонь/#3 (Сборщик live-PIE, CU есть).
3. **UE-автотесты боевого кора** (headless, БЕЗ CU) — план в этом файле выше/в STATUS, Ринат одобрил направление; итерация A=каркас+движение+навмеш, B=погоня, C=лут/квесты. Снимет ночной QA с CU. Пороги — draft, помечать Ринату.
4. Полировка: #21 броня 2×3 (меши phase4 есть, нужен Legs_02 от modeler + статы), #18 HUD-иконки (нужны текстуры), #22 лица NPC (НУЖЕН арт-ориентир Рината).
5. DT_Quests/DT_Items (конструкторы) — ближе к концу демки (JIT).

## DEBUG-КЛАВИШИ: F1 cam,F2 items,F3/F4 броня,F6 use,F7 drop,F9/F10 buy/sell,F12 clearsave,M+100,T торговец,Y староста,G взять квест,H сдать,K+волк,J god,U force-drop,B тест-волк вплотную,O оверлей,N force-kill ближнего,V телепорт-Логово,Z телепорт-база,C 5 шкур,X ноутбук,P убить игрока + слайдер магазина (стрелки/колесо ±1, Shift±10, Enter подтвердить).
...
---
# === SAVE 2026-06-16 ~00:05 — конец сессии, СТАРТ ЗАВТРА ОТСЮДА ===

## РЕЖИМ РАБОТЫ (зафиксирован Ринатом)
- MVP-демку доводим ПО ЭТАПАМ, **СНАЧАЛА ГЕЙМПЛЕЙ, ПОТОМ ВИЗУАЛ**. Ринат принимает каждый этап.
- Этапы геймплея: 1) WASD+камера → 2) бой+погоня → 3) квесты → 4) смерть/выживание. **Визуал ЗАМОРОЖЕН**, пока геймплей не пройден.
- Приёмка геймплей-этапа — ТОЛЬКО живой PIE с пруфом (лог QA:/глаз Рината), НЕ headless-параметры.
- Скоуп демки: деревня + база бандитов + логово волков + 2 квеста. Не золотить.

## 🔴 ГЛАВНОЕ НА ЗАВТРА: ДОБИТЬ WASD (Этап 1) — зажат в угол
Сага (по реальным PIE-логам Рината):
1. Корень НЕ NavWalking (mode=1 Walking уже изначально — гипотеза cpp была неверной).
2. Камера-фикс `f929da5`: направление движения считалось от `GetCameraRotation().Yaw`; после фикс-изометрии (pitch −60) базис вырождался → `MoveDir=0`. Исправлено: направление от `MovementBasisYaw=90` (камера-независимо, плоскость XY). Лог подтвердил: теперь `MoveDir=(0,1,0)` ✓.
3. **ОСТАЁТСЯ (добить):** `vel2D=0` при `MoveDir=(0,1,0)`, `mode=1`, `onGround=1`, `maxWS=600`, `simPhys=0`, `nPlayers=1` → ввод ИГНОРИТСЯ или скорость ГАСИТСЯ после AddMovementInput. ГЛАВНЫЙ подозреваемый (наводка Рината!): `SetIgnoreMoveInput(true)` оставлен debug-тулингом (телепорты V/Z, F-клавиши, god/камера-режим/лок-цель), либо per-tick StopMovementImmediately/DisableMovement/SetActorLocation-пиннинг. cpp грепал Source на момент конца сессии (см. его context/коммит).
   ЗАВТРА: прочитать итог cpp, найти и убрать «держатель» движения, релинк (редактор закрыт), Ринат жмёт W → ждём vel2D>0.
- Диаг-лог `QA: MOVE ...` в `ContrarySurvivorPlayerController::Move()` (TEMP, убрать после фикса). cpp добавлял поля ignoreMove/ctrlInput/vel.
- ⚠️ Авто-тесты `ContrarySurvivor.Movement.*` (NavWalkingFixed/TranslatesOnInput) — FAIL, но НЕНАДЁЖНЫ headless (NavWalkingFixed тестит неверную гипотезу; TranslatesOnInput зовёт AddMovementInput минуя Move() + CMC в transient-мире не едет). НЕ доверять им по движению — верификация = live PIE.

## ВИЗУАЛ (Этап 3) — сделан с UE-скринами, но ЗАМОРОЖЕН до прохождения геймплея
- ✅ Костёр тёплый (был синий свет — `unreal.Color` BGRA переставил R↔B), кровь-лужи (были розовые плиты — `unreal.Rotator` roll вместо pitch), SkyLight+SkyAtmosphere+экспозиция, дорога→декаль. Все с реальными UE-скринами (S3_FINAL_*.png в Saved/Screenshots/WindowsEditor).
- 🟠 ХВОСТ: дом на UE-скрине ВСЁ ЕЩЁ мутно-серый (vcol слабо тянет, вероятно Power(2.2) пере-обесцвечивает коричневый под небом) — доводка позже (Этап визуала).
- Модели приняты+экспортированы: дом-изба (крыша ровная, vcol), забор, колодец, кости+кровь, трава зелёная, одежда L1/L2, кепка бандита v2.
...
- ВИЗУАЛ-ПРУФ — только реальный UE-скрин (фронтальный свет), не headless-параметры (правило записано выше).

## КАНАЛЫ СВЯЗИ — РИНАТ ПЕРЕСОБИРАЕТ С НУЛЯ (важно!)
- ВСЕ фоновые watcher'ы/пульсы УБИТЫ по приказу Рината (heartbeat-цикл, INBOX-watch, TEST_REPORT-watch). `HEARTBEAT.md` УДАЛЁН из git + gitignored. **НЕ поднимать будильники завтра, пока Ринат не задаст НОВУЮ схему связи.**
- Текущая (до пересборки) пара: Ринат → `INBOX.md`, game-lead → `STATUS.md`, Сборщик → `TEST_REPORT.md`. `GL_SIGNAL.txt`/`QA_SIGNAL.txt` — LEGACY (старый ночной дубль), под удаление.
- Сборщик CU-зависим: live-PIE невозможен без присутствия Рината (request_access таймаутит). Headless — сам.

## ПРОЦЕСС-УРОКИ (Ринат указал на кризис)
- Параллелить ≤3 агента, не гонять по одному; отдавать готовую волну на тест СРАЗУ, не сериализовать.
- Не распыляться: один приоритет (сейчас — WASD/геймплей), визуал заморожен.
- Не говорить «готово» без правильного пруфа (геймплей=PIE, визуал=UE-скрин). Пульс ≠ прогресс.
