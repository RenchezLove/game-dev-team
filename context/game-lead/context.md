...
- **БАГ 2 — бандит «серый + перевёрнутый».** (а) Серый: vertex-color флэт-материал из Blender не импортирован → дефолт серый (поэтому «изменений не видно» — облик гопника цветовой). (б) Поза/ориентация: модульный риг (Leader Pose+AnimBP+трансформы) живёт в BP игрока, не в C++; BP бандита его не имел. **ЧАСТИЧНО ИСПРАВЛЕНО (cpp d86b324, enemy-only):** PostInitializeComponents `Torso/Legs SetLeaderPoseComponent(Head)`; конструктор `GetMesh()->SetRelativeLocationAndRotation((0,0,-90),(0,-90,0))`. **ОСТАЛОСЬ editor-side (раунд 3):** назначить Head AnimBP бандиту (ABP_HumanoidCharacter), создать M_Bandit_VColor (VertexColor→BaseColor) + переимпорт vertex-colors + назначить мешам; подтвердить, что ориентация выправилась (иначе переэкспорт modeler с верными осями Blender→UE).
...
## NEXT (для свежей сессии game-lead, старт отсюда)
1. (Опц.) Ринат может Play текущий редактор: подтвердить, что бандит теперь получает урон/умирает (фикс коллизии) + сообщить, выправилась ли ОРИЕНТАЦИЯ (фикс C) или нужен переэкспорт modeler.
2. Спавнить unreal-operator (тулзы должны инжектнуться в свежей сессии): раунд 3 — Head AnimBP бандиту + материал M_Bandit_VColor/vertex-color + проверка ориентации + скрин. Ветка `feature/phase1-enemy-stats`.
3. (Опц.) Build Lighting чтобы убрать «ярко-зелёный unlit» вид.
4. **qa-ревью ветки** `feature/phase1-enemy-stats` (независимая сборка + аттест с артефактом) → merge в master ГЕЙМ-репо (вручную, своего хука-гейта там нет). Перед этим — РЕШЕНИЕ Рината по версионированию Content/ (сейчас /Content в .gitignore — ассеты не в git; вариант A=Git LFS рек., B=диск+бэкап).
5. Старт Фазы 2 по роудмапу.
