...
## 2026-08-12 — сессия «переезд запуска + окно Поддержать автора» (закрыта)

**Переезд запуска на пустой уровень (коммит `52313db`).** Игра стартует на `L_Boot` (в нём 2 актора — свет и точка появления, режим игры в настройках уровня НЕ задан; headless-дамп 08-12 07:38). Первый запуск после установки — сразу в мир, следующие — меню. Устройство: `Modes/BootGameMode` (без пешки и HUD, контроллер мягкой ссылкой) + `Subsystems/GameFlowSubsystem` (намерение перехода, читается один раз) + строка в `Config/DefaultEngine.ini:119`. Сборка `Saved/build-boot-0812-d.log`, 6 тестов `ContrarySurvivor.BootFlow` зелёные.
- ⛔ Факт: в строке `+GameModeMapPrefixes=(Name="L_Boot",GameMode="...")` поле называется `GameMode`, а не `Value` (`GameMapsSettings.h:56`).
- Порядок выбора режима игры в UE 5.5 (`GameInstance.cpp:1490-1568`): настройки уровня → `?game=` → префикс имени карты → глобальный. Правило по префиксу не задевает `L_World_C` (у него режим в настройках уровня).
- Развилки, решённые лидом: музыка при выходе из паузы в меню обрывается (мир обязан выгружаться); на загрузочном уровне ЧЁРНЫЙ кадр вместо пустого неба.

**Окно «Поддержать автора» — полный разбор в ADR-070 (+ дополнение).** Коммиты `d2462a1`, `7cbe386`, `e6fd856`. Принято Ринатом живьём: «Окно меня устраивает».

**Витринные кадры для магазина (коммит `345f3b2`, 7 штук в `reports/store-screenshots/`).** `01-boy`, `02-derevnya`, `03-inventar`, `04-magazin`, `06-kvest`, `07-menyu`, `08-razgovor`, все 1920x1080. Кадра `05-mir` (открытый мир) нет и не будет прежним способом: камера строго сверху, дали не показывает, за деревней пустое поле. Нужен либо красивый уголок карты, либо отдельная камера.
- Способ съёмки: игра поднимается как игра (`UnrealEditor.exe <uproject> -game -windowed`), кадр берётся ВМЕСТЕ с окнами интерфейса. Ловушка: игра игнорирует размер окна из строки запуска и берёт своё сохранённое разрешение — файл настроек подменять и возвращать. Запуск игры лиду блокирует classifier при вызове через `&`, проходит через фоновый запуск. Оператору запуск запрещён: игру поднимает лид, оператор снимает.
- Разведка оператора: (а) снимок с показом окон пишет кадр РОВНО размера игрового окна, обычный «снимок высокого разрешения» не годится (рисует мимо экранных окон, а рюкзак и магазин — окна); (б) мобильный вид на компьютере включается сам, слой пальцевых кнопок уже включён; (в) невидимый режим не годится — нажатия не доходят; (г) переносов в игре ровно два: Y к старосте, T к торговцу; клавиш Z и V нет; F3 надевает броню, F4 снимает.

**Волна 08-09 (влита в ветку, закрыта):** полосы состояния `b8e133c`; стик `81b8644`; вид меню `46a2d39`+`294274b`; настройки укрупнялись и откачены `111ae3d`→`c0f285b`; признак «окно принадлежит владельцу» `df92e9e`; пауза `a5ce8bf`; строка задачи вступления `32197b0`; звуки мира под меню `02fc412`+`deadb9f`; трек `2fa7f32`, музыка паузы `496fb36`, упаковка звука `384cb2f`; баланс `c08b236`; адрес Telegram в кавычках `c22b946`; тесты адресов `bf1e1c8`. Пакет `gl-pack-0809-v5.log` BUILD SUCCESSFUL, установлен на телефон.
- **Съёмка ЖИВОЙ игры с нажатиями (урок оператора 08-12):** кадр брать захватом области экрана средствами PowerShell; программе-снимальщику ОБЯЗАТЕЛЬНО объявить, что она знает про масштабирование экрана, иначе координаты приходят уменьшенными и нажатия уезжают мимо; перед КАЖДЫМ нажатием проверять, что под точкой именно окно игры (иначе игра уходит за чужие окна и в кадр попадают посторонние документы Рината — так и вышло, кадр удалён сразу). Сам оператор записать это себе в память не смог: запись напарника в служебную папку настроек запрещена.

## Волна «Чтобы не бесило» (29–30.08.2026) — ЗАКРЫТА, принята Ринатом живьём
Ветки: `feature/search-inventory-0829` (`1f6c49b`) и `feature/combat-music-0830` (`a765738`, поверх неё правки контента). Обе НЕ слиты в master.
- Мигание кнопки перезарядки при магазине ≤1 патрона (`d30cd55`, ADR-079), настройки в Class Defaults `WBP_TouchControls`.
- Отладочные клавиши 1 (+100 денег) и 2 (снимок игровой области) — только привязки к уже готовым действиям (`483923a`, ADR-080).
- Тексты и вид индикатора хромоты переехали с игрока в окно `WBP_LimpIndicator` (`24b2d89`, ADR-081).
- Обыск и торговля — режимы одного экрана из двух панелей (`a0fdc74`, `62893ff`, `2871ce3`, ADR-082/084); мини-окно сделки слоем выше (ADR-085).
- Боевая музыка: событие входа в бой + опрос реестра ИИ радиусом 2500, дак фона 0.7 (`4870c8a`, `a765738`, ADR-086). Сборка 21/21, тесты 4/4+3.
- Звук выстрела 1911 с вариациями (`eb00e9e`, ADR-087).
Правки Рината в окнах и карте сохранены коммитами `9dfb1e2`, `8d99014`, `e694c03`, `d875bdf`, `354e873`, `f6dfc6e`, `2c79ad3`, `4a66c44`, `c348102`.

## 2026-09-24…26 — ADR-088 (таблица предметов) и выпуск 0.2.2 (5) — вынесено из «СЕЙЧАС» 26.09
**26.09 итог:** ветка на голове `9edc480` (выложена, ls-remote сверен). Три чертежа пересохранены, пустая картинка подтверждена обратным чтением в новом запуске. Тесты 191/0 (`Saved/test-items-icons-0926.log`), сохранение Рината восстановлено из копии, хеш совпал. qa: ДОБРО (сборка с нуля `Saved/qa-build-items-0926.log` EXIT=0). Сам выгрузил DT_Items: 17 строк, у всех есть картинка и файл существует. Замечания qa (не блокеры): без строки в таблице предмет молча без картинки; магазин без таблицы — без картинок. Пункт «cpp п.2» ниже ЗАКРЫТ.
**Дальше (26.09):** ПРИНЯТО Ринатом живьём, master игры = `9edc480` (слито перемоткой, выложено, ls-remote сверен). Плановую таблицу не трогал — ждёт его слова.

**Телефон 26.09:** на телефоне стояла ПРОВЕРОЧНАЯ сборка (отладочный ключ, 0.1.1/3 от 06.09) — магазинная поверх не встаёт (подписи разные). Поставлена проверочная 0.2.1/4 из master `9edc480` (`pack-android/logs/pack-dev-0926.log`), сохранение цело (хеш до/после `efe6ddf6…`), копия — `Saved/PhoneBackup-0926/`. Магазинная 0.2.1/4 тоже собрана (`pack-shipping-0926.log`, подпись боевым ключом сверена, debuggable=false, без разрешений хранилища). Версия 4/0.2.1 — локальная правка Рината в `Config/DefaultEngine.ini`, в git НЕ закоммичена.
**⚠️ ОТМЕНЕНО: в RuStore УЖЕ была 0.2.1/4 (магазин отверг). Актуальная магазинная — 0.2.2/5, ветка `release/0.2.2-build5` = `69d0656`, пак `pack-shipping-0926-c.log` BUILD SUCCESSFUL, подпись боевая, файл `…/Утвержденные/Marevo-0.2.2-build5-release.apk`. УРОК: перед повышением версии смотреть страницу игры в RuStore (versionName в HTML). Удаление метки `release/0.2.1-build4` и файла build4 запрещено системой — оставлены.** Ниже — история: ветка `release/0.2.1-build4` = `2961661` (версия 4/0.2.1 без паролей + стик Рината в WBP_TouchControls; стик двигается числами «Отступ центра стика от угла», код ApplyStickCornerLock перекрывает дизайнер). Метка `release/0.2.1-build4` выложена. Пак `pack-android/logs/pack-shipping-0926-b.log` BUILD SUCCESSFUL, подпись боевая сверена, debuggable=false, хранилища нет. Файл: рабочий стол `Скриншоты ContrarySurvivor/Витрина 2026-08-15/Утвержденные/Marevo-0.2.1-build4-release.apk`. НЕ СДЕЛАНО: слияние ветки release в master через qa.
**Следующее обновление (Ринат 26.09):** единый размер плиток предметов во всех окнах (инвентарь/обыск/магазин): общая настройка размера, число колонок по ширине окна, свободное место поровну, проверить поля на картинках. План одобрен на словах, делать в следующем обновлении.

**Ветка игры:** `feature/items-single-source-0924` (от master `c233243`). Слита в master 26.09.
**Коммиты ветки:** `36149d2` cpp — единая точка наложения строки DT_Items (`AMasterInventoryItem::PostInitializeComponents`), новая `ContraryItems::SpawnItem`, убран `AConsumableItem::GetDefaultIcon`; сборка 0 (`Saved/build-items-single-source-0924-b.log`), тесты 191/0 (`Saved/test-items-single-source-0924-b.log`) — сверил лично. `3d1e9de` я — 18 новых иконок (русский стиль, листы ChatGPT Рината) заменены под старыми именами T_Item_*, 256x256, настройки старых скопированы; удалены 12 неиспользуемых (T_Icon_Armor_* x9, T_Icon_Knife, T_Icon_Pistol, Icon_Knife2) после проверки ссылок реестром; выгрузка обратно просмотрена. Нарезка и скрипты — `assets/ui-icons/items-0924/`. `a8094c7` unreal — лут бандита (ItemRow), пикапы и машины L_World_C на строки; обратное чтение в scratchpad `items0924/dump_after.txt` (временная папка!). Пикапам в список добавлены патроны ammo_9mm с прежним числом (список отключает одиночные поля).
**cpp п.2 (24.09):** код без прошитых картинок закоммичен мной как WIP `860deb5` и выложен (сборка 0 `Saved/build-items-icons-0924-b.log`, тесты НЕ прогонялись). НЕ СДЕЛАНО: пересохранение BP_Ammo9mm/BP_Knife/BP_Pistol (служебный запуск движка PID 21640 + CrashReportClientEditor 21140 завис с 23:06; снять cpp не дали права — вопрос к Ринату; память забил llama-server Рината), затем полный прогон тестов с копией сохранения. Текущие версии трёх чертежей cpp закоммитил как WIP `95e2e45` (выложено), пустая картинка в них НЕ подтверждена — после пересохранения закоммитить заново. Скрипты cpp: resave_bp_icons.py, readback_bp_icons.py (временная папка cpp). Запасную картинку при ненайденной таблице НЕ добавляли. убрать прошитые картинки в конструкторах AAmmoItem/AMeleeWeapon/APistol/AArmorTiers и AQuestItem::GetItemIcon; пересохранить BP_Ammo9mm/BP_Knife/BP_Pistol (они сейчас висят незакоммиченными пересохранениями от unreal — откатывать мне/агентам запрещено) и закоммитить с кодом.
**Копия сохранения Рината:** `ContrarySurvivor/Saved/SaveBackup-0924/`; эталон ContrarySave.sav sha256 `bc5178446c2673fc8a4aaa84b8fe221a759a1d01d9322479bfe31aafac89f708`. Тесты по-прежнему пишут в боевой слот — сверять после каждого прогона.
**Дальше:** qa-гейт ветки (сборка с нуля + тесты + сверка сохранения) → приёмка Ринатом живьём (иконки в инвентаре/магазине, мешки, машины, труп бандита) → слияние по его слову.
