...
**Следующий повод подняться:** старт исполнения Этапа B/C — пакет из 19 вопросов Ринату (попап закрыт; NPC 6 + локации 5 + крыша-тон 1 + земля/дорога 3 + … ) game-lead подаст, когда дойдём. Попап — закрыт (финал отдан cpp). Опц.: перерисовать monetary-иконку без ₽, если cpp попросит.
